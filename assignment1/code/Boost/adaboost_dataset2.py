...
param_range = np.linspace(1, 1000, 10).astype(int)
...
title = "Learning Curves (Boosted Decision Tree, Estimator = 700)"
# Cross validation with 100 iterations to get smoother mean test and train
# score curves, each time with 20% data randomly selected as a validation set.
decisionTree = DecisionTreeClassifier(criterion = 'entropy', random_state = 0, max_depth = 3)
classifier = AdaBoostClassifier(base_estimator=decisionTree, random_state = 0, n_estimators = 700)
plt, train_scores_mean, test_scores_mean = plot_learning_curve(classifier, title, X, y, ylim=(0.5, 1.1), cv=5)

plt.savefig('learn_dataset2')
